<template>
  <div class="bg-[#2a2a2a] min-h-screen">

    <div class="mx-auto max-w-2xl px-4 py-16 sm:px-6 sm:py-24 lg:max-w-7xl lg:px-8">
      <h2 class="sr-only">Products</h2>
      <img class="h-20 bg-white rounded-full p-4" :src="logo" alt="Your Company" />
      <div class="mt-12 mx-auto max-w-2xl lg:mx-0">
        <h2 class="text-5xl font-semibold tracking-tight sm:text-7xl text-white">Shop</h2>
        <p class="mt-8 text-lg font-medium text-pretty sm:text-xl/8 text-gray-300">
          Browse Our Latest Masterpieces – Paintings, Sculptures & More
        </p>
      </div>
      <div class="mt-12 grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 xl:gap-x-8">
        <a v-for="product in products" :key="product.id" :href="product.href" class="group">
          ...
          <h3 class="mt-4 text-sm text-gray-300">{{ product.name }}</h3>
